...
void MCalibration::Calibration_OpenCV_TCP(std::vector<KMat<double>>& _vec_inputPoseMat, KMat<double>& _outputMat)
{
	...
	for (int i = 0; i < _vec_inputPoseMat.size() - 1; i++)
	{
		...
		DeRotateMat = DeRotateMat._Normalize();

		KMat<double> DeTranslateMat(3, 1);
		DeTranslateMat={
			{_vec_inputPoseMat[i](0, 3) - _vec_inputPoseMat[i + 1](0, 3)},
			{_vec_inputPoseMat[i](1, 3) - _vec_inputPoseMat[i + 1](1, 3)},
			{_vec_inputPoseMat[i](2, 3) - _vec_inputPoseMat[i + 1](2, 3)}
			};
...
void MCalibration::LeastSquareSolveTCP(std::vector<KMat<double>>& _vec_inputPose, KMat<double>& _matOut)
{
	...
	// 构造矩阵AE
	 // 构造矩阵 A 和向量 b
	Eigen::MatrixXd A(16, 16); // 4 * 4 = 16
	Eigen::VectorXd b(16);
	for (int i = 0; i < 4; ++i) {
		A.block<4, 4>(i * 4, 0) = vec_EigenMat[i].block<4, 4>(0, 0);
		b.segment<4>(i * 4) = vec_EigenMat[i].block<4, 1>(0, 3);
	}
